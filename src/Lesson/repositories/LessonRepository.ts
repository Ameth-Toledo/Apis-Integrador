...
export class LessonRepository {
    ...
    public static async createLesson(lesson: Lesson): Promise<Lesson> {
        const { module_id, title, content, progress, created_by, created_at, updated_by, updated_at, deleted } = lesson;
        const query = `INSERT INTO lesson (module_id, title, content, progress, created_by, created_at, updated_by, updated_at, deleted) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`;
        const values = [module_id, title, content, progress, created_by, created_at, updated_by, updated_at, deleted ? 1 : 0];
    ...
    public static async updateLesson(lessonId: number, lessonData: Lesson): Promise<Lesson | null> {
        const { module_id, title, content, progress, updated_at, updated_by, deleted } = lessonData;
        const query = `UPDATE lesson SET module_id = ?, title = ?, content = ?, progress = ?, updated_at = ?, updated_by = ?, deleted = ? WHERE lesson_id = ?`;
        const values = [module_id, title, content, progress, updated_at, updated_by, deleted ? 1 : 0, lessonId];
